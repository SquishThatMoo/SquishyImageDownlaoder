            ...
            getCurrentTab().then(function(tab) {  
                if (tab !== undefined || tab != null) {
                    ...
                    switch (domain) {
                        ...
                        case "www.google.at":
                            matchedDomain = true;
                            chrome.tabs.sendMessage(tab.id, {squishImageSaver: {scrapeGallery: true, scrape: domain, settings: currentSettings}});
                            break;
                        default:
...
*/
    ...
    function downloadMedia (mediaItem) {
        if (mediaDataNotEmpty(mediaItem)) {
            console.log(mediaItem, createFileName(mediaItem));
            return new Promise(resolve => chrome.downloads.download({
                conflictAction: 'uniquify',
                filename: createFileName(mediaItem),
                url: mediaItem.href,
    ...
    function mediaDataNotEmpty (mediaItem) {
        return (mediaItem?.onlyTitle && mediaItem?.title != '') || mediaItem?.href;
    }
    ...
    function createFileName (mediaItem) {
        ...
        if (mediaItem?.onlyTitle) {
            fileNameString = "$title$";
        }     
