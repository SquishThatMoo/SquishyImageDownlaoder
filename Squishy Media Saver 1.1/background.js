...
squishImageSaver = (() => {
                ...
                if (squishImageSaver?.mediaData) {
                    ...
                    if (!(squishImageSaver.mediaData?.notSupported === true) && (!openEditor || squishImageSaver?.finalDownload)) {
                        if (squishImageSaver?.downloadInExtension) {
                            console.log("Download media in extension.");
                            downloadMedia(mediaItem);
                            downloaded++;
                        } else {
                            console.log("Catch external download.");
                            changeDownloadFilename(mediaItem);
                            downloaded++;
    ...
    function changeDownloadFilename (mediaItem) {
        if (currentSettings.scraping.deviantart.renameButtonDownload && mediaDataNotEmpty(mediaItem)){
            let filenameChangeHandler = (item, suggest) => {
                try {suggest({
                    filename: downloadSubFolder + createFileName(mediaItem),
                    conflictAction: 'uniquify'});
                } catch (e) { 
                    console.log("Error changing file name", e);
                }
                chrome.downloads.onDeterminingFilename.removeListener(filenameChangeHandler);
            }
            chrome.downloads.onDeterminingFilename.addListener(filenameChangeHandler);
        }
    }
