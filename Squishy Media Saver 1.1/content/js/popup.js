...
window.onload = () => {
    ...
    const checkboxes = {
        deviantart: {
            ...
            renameButtonDownload: new mdc.checkbox.MDCCheckbox(document.getElementById('mdc-da-ui-download-filename')),
        },
        e621: {
    ...
    function setSettings(settings) {
        ...
        checkboxes.deviantart.renameButtonDownload.checked = settings.scraping.deviantart.renameButtonDownload;
    ...
    function onSettingUpdate(event) {
        ...
        switch(event.target.id) {
            ...
            case 'da-ui-download':
                ...
                break;
            case 'da-ui-download-filename':
                isValid = true;
                currentSettings.scraping.deviantart.renameButtonDownload = checkboxes.deviantart.renameButtonDownload.checked;
                break;
...
function sendSettingsToStorage(settings) {
    ...
    localStorage.setItem('squishyMediaDownloaderSettings', JSON.stringify(settings));
}
...
function validateSettings(settings) {
    ...
    if (settings.scraping) {
        ...
        let renameButtonDownload = toBool(settings.scraping.deviantart.renameButtonDownload);
        if (renameButtonDownload == null) {
            settings.scraping.deviantart.renameButtonDownload = defaultSettings.scraping.deviantart.renameButtonDownload;
        } else {
            settings.scraping.deviantart.renameButtonDownload = renameButtonDownload;
        }
...
function sendToBackend(payload, result) {
    chrome.runtime.sendMessage(id, {squishImageSaver: payload}, result);
}
